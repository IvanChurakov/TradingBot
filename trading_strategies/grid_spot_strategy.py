...
class GridSpotStrategy(BaseTradingStrategy):
    ...
    def get_price_zone_info(self, current_price: float) -> str:
        if not self.__is_price_within_grid(current_price):
            return f"Current price {current_price:.2f} is **outside** the grid range."
        ...
        elif thresholds.upper_grid is not None and thresholds.upper_grid >= current_price > thresholds.upper_sell_threshold:
            info += f"Zone: SELL zone (between {upper} and {thresholds.upper_sell_threshold:.2f})"
        else:
            info += "Zone: TRADE zone (no action)"
